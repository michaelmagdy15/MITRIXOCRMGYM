...
export const checkExpiredMemberships = onSchedule(
  ...
  async (event) => {
    ...
    try {
      ...
      const clientDocs: admin.firestore.QueryDocumentSnapshot[] = [];
      ...
      for (const doc of clientDocs) {
        ...
        if (client.membershipExpiry) {
          ...
          const hasActiveFuture = client.packages.some((pkg: any) => {
            if (pkg.endDate) {
              ...
              if (scheduleId) {
                ...
                if (schedDoc.exists) {
                  ...
                  if (sDateTime && sDateTime >= today) {
                    ...
                    const remainingAttendees = (sData.attendees || []).filter((id: string) => id !== doc.id && id !== client.memberId);
