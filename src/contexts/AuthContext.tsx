...
interface AuthContextType {
  ...
  inviteUser: (email: string, role: UserRole, displayName?: string, phone?: string, permissionTemplateId?: string, options?: userService.InviteUserOptions) => Promise<void>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const approvePasswordResetRequest = async (id: string, email: string) => {
    const isSyntheticEmail = email.endsWith('@mitrixogymcrm-member.local');
  ...
  const inviteUser = async (email: string, role: UserRole, displayName?: string, phone?: string, permissionTemplateId?: string, options?: userService.InviteUserOptions) => {
    await userService.inviteUser(email, role, displayName, phone, permissionTemplateId, options);
  };
