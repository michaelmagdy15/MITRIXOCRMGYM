                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      {daySessions.length > 0 && (
                        <span className="text-[10px] font-bold text-muted-foreground opacity-60">
                          {daySessions.length} {language === 'ar' ? 'جلسة' : 'sessions'}
                        </span>
                      )}
                    </div>
                      ...
                      {daySessions.slice(0, 3).map(session => {
                        ...
                        return (
                          <div 
                            ...
                            className={`px-1.5 py-0.5 text-[10px] font-medium rounded-md border truncate shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer ${getStatusColor(session.status)}`}
                          >
                            {format(parseISO(session.date), 'HH:mm')} - {client?.name || 'Member'}
                          </div>
                        );
                      })}
                      {daySessions.length > 3 && (
                        <div className="text-[9px] font-bold text-center text-primary pt-0.5">
                          + {daySessions.length - 3} {language === 'ar' ? 'المزيد' : 'more'}
                        </div>
                      )}
                    </div>
                  </div>
                        ...
                        daySessions.map(session => {
                          ...
                          return (
                            <div 
                              key={session.id}
                              onClick={() => setSelectedRecord(session)}
                              className={`p-2 text-xs font-semibold rounded-xl border space-y-1 shadow-sm transition-all hover:scale-[1.02] cursor-pointer ${getStatusColor(session.status)}`}
                            >
                              <div className="flex justify-between items-center">
                                <span className="font-bold text-[10px]">{format(parseISO(session.date), 'h:mm a')}</span>
                              </div>
        ...
        <DialogContent className="max-w-md bg-background/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-6">
          <DialogHeader className="pb-2 text-left">
            <DialogTitle className="text-xl font-bold flex items-center gap-2">
              ...
              <span>{language === 'ar' ? 'حجز حصة تدريبية جديدة' : 'Book a Training Session'}</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4 text-sm">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <Label className="text-xs font-bold text-muted-foreground uppercase">{language === 'ar' ? 'الوقت' : 'Time'}</Label>
                <input 
                  type="time"
                  className="flex h-10 w-full rounded-xl border border-white/5 bg-muted/20 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
                  value={bookTime}
          ...
          <DialogFooter className="pt-4 border-t flex flex-col sm:flex-row gap-2">
            ...
            <Button 
              ...
              disabled={!bookClientId || !bookDate || !bookBranch}
            >
