...
### Fixed This Session
**PRD.1 1-on-1 PT Sessions & Capacity Flow** - FIXED ✅
- Updated PT_CAPACITY_LIMITS in src/types.ts to strictly enforce min/max bounds (e.g. Small Group 3-5 participants).
- Rewrote updatePTPackageRecord in src/hooks/usePTSessions.ts with atomic deduction logic (only deducts on Attended/No Show, safely restores on Rescheduled/Scheduled).

**PRD.2 Reporting Exports (PDF, XLSX, CSV)** - FIXED ✅
- Added jspdf, jspdf-autotable, and xlsx libraries.
- Upgraded AdvancedReports.tsx and Reports.tsx to utilize a unified Export Dropdown Menu for native CSV, PDF, and XLSX export functionality.
- Added CSV Export to the Nutrition Manager Analytics tab.

**PRD.3 CRM Case Management (SLAs & Escalations)** - FIXED ✅
- Added 48-hour SLA deadline (slaDeadline) logic to Complaints.tsx.
- Added visual indicators for Overdue (breached SLA) and Escalated tickets.
- Implemented an "Escalate to Manager" capability inside the complaint details.

**PRD.4 Operational Exceptions & Maker-Checker workflow** - FIXED ✅
- Upgraded uditService.ts to strictly enforce a mandatory eason argument for OVERRIDE or ADJUSTMENT actions.
- Built a manual "Adjust Balance" interface in EntitlementManager.tsx that requires a justification and routes through the secure Manager approval workflow.


## RECENT WORK â€” 2026-09-06 (Inzan PT & Classes Audit, Payment Mirroring & Renewals, Zero Invalid Time Value)

### Fixed This Session
**INZAN.1 Complete PRD Audit & PT/Classes Dual-Write Sync** â€” FIXED âœ…
...
**PAY.1 Unbroken Package-to-Payment Mirroring & Seamless Renewals** â€” FIXED âœ…
...
**DATE.1 Platform-Wide Zero "Invalid Time Value" Hardening** â€” FIXED âœ…
...
**ISO.1 Multi-Tenant Isolation & Strike Gym Integrity Verification** â€” VERIFIED âœ…
...
## RECENT WORK â€” 2026-08-23 (Mobile App UX Overhaul & Interactive Booking Flow)

### Fixed This Session
**MOB.1 Mobile App Preloader & Splash Optimization** â€” FIXED âœ…
...
**MOB.2 Storefront Corporate & Points Cleanup** â€” FIXED âœ…
...
**MOB.3 Smart Multi-Criteria Storefront Filters** â€” FIXED âœ…
...
**MOB.4 Interactive Class Booking Dialog with Package Upsell** â€” FIXED âœ…
...
**MOB.5 Instant Real-Time Schedule Sync (Admin Calendar <-> Mobile App)** â€” FIXED âœ…
...
## RECENT WORK â€” 2026-08-23 (Phone Auth SMS OTP, Staff Phone Management, Member Multi-Format Login)

### Fixed This Session
**AUTH.1 Automated Phone Auth & SMS OTP Password Reset** â€” FIXED âœ…
...
**AUTH.2 Staff & Coach Phone Number Management** â€” FIXED âœ…
...
**AUTH.3 Member Multi-Format Login & Self-Linking** â€” FIXED âœ…
...
**DOCS.1 Comprehensive Guides & Flowcharts** â€” COMPLETED âœ…
...
## RECENT WORK â€” 2026-08-21 (PT Edge Cases & Session Management)

### Fixed This Session
**PT.1 PT Session Booking Edge Cases** â€” FIXED âœ…
...
**PT.2 Admin/Coach Requests Dashboard** â€” FIXED âœ…
...
**PT.3 Build/Lint Health** â€” FIXED âœ…
...
## CRITICAL GAPS â€” Audit 2026-08-18 (Member Portal Permissions)

### Fixed This Session

**C5. Member login broken ("missing or insufficient permissions", member 624)** â€” FIXED âœ…
- Root cause: commit 703db01 restricted `users` reads to owner+staff; `loginWithMemberId` queried `users where clientRecordId == id` BEFORE auth â†’ permission denied
...
**C6. Member-facing reads of protected collections moved server-side** â€” FIXED âœ…
- `loginWithCoachId` â†’ new public `POST /api/coach/resolve-email` (server-side 3-step lookup)
- `submitMemberPasswordResetRequest` â†’ new public `POST /api/member/request-password-reset` (server-side phone verify + rate limit)
- PT coach list in MemberSessions â†’ new public `GET /api/member/coaches` (id/name/branch only, no emails)
- New public `POST /api/attendance/self-checkin` â€” full check-in math (PIN, member lookup, double-check-in, decrement) moved to server; kiosk no longer needs anonymous sign-in or client rule grants

**C7. Server-authoritative session math (members can no longer self-grant sessions)** â€” FIXED âœ…
...
**C8. Staff escalation via users self-create closed** â€” FIXED âœ…
- `users` create rule: self-create now only role `'client'` (was `'client' or 'coach'` â€” any authenticated user could self-promote to coach = staff)

**C9. `/api/clients/update` role check** â€” FIXED âœ…
...
**C10. Notifications collection had no rules** â€” FIXED âœ…
...
**C11. registerFreeUser (Join Club) broken for new users** â€” FIXED âœ…
- Scanned all `clients` to compute the next MEM- ID â€” denied for brand-new users (no users doc yet)
- Now uses atomic `counters/memberIds` transaction

**C12. Baseline lint errors fixed** â€” FIXED âœ…
...
### Public endpoints added (documented per AGENTS.md Â§5)
- `POST /api/member/resolve-email` â€” resolve auth email by member ID (pre-auth login/fallback)
- `POST /api/coach/resolve-email` â€” resolve auth email by coach ID/name (pre-auth login)
- `POST /api/member/request-password-reset` â€” member password reset request (pre-auth)
- `GET /api/member/coaches` â€” coach list for member portal (id/name/branch only)
- `POST /api/attendance/self-checkin` â€” PIN-validated self check-in (kiosk, pre-auth)
...
## CRITICAL GAPS â€” Audit 2026-08-18

### Fixed This Session

**C1. Branding settings not loading** â€” FIXED âœ…
...
**C2. Logo upload not persisting** â€” FIXED âœ… (same root cause as C1)
- Logo uploads to Firebase Storage worked correctly
- But GET /api/settings never loaded the saved logoUrl
- Fixed by C1 fix

**C3. CockroachDB removed** â€” FIXED âœ…
- All club operations migrated from CockroachDB to Firestore:
  - juiceBarOrders â†’ Firestore collection
  - lockers â†’ Firestore collection
  - lockerRequests â†’ Firestore collection
  - guestInvites â†’ Firestore collection
  - auditLogs â†’ Firestore collection
- Server startup no longer tests CockroachDB connection
- Imports removed from server.ts

**C4. Second preloader disabled** â€” FIXED âœ…
...
**Status:** Deferred â³ (Will be published later by owner)
...
1. **[P1] PT / Session Management System** â€” Build comprehensive session and capacity management per INZAN_PT_MANAGEMENT_PRD.md
2. **[P2] Push notifications enhancement** â€” richer notifications with actions
